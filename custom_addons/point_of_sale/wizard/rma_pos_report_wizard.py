...
class RmaPosReportWizard(models.TransientModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        rmas = self.env['joyeria.reparacion'].search([('fecha_retiro', '>=', date_start), ('fecha_retiro', '<=', date_end)])
        poses = self.env['pos.order'].search([('date_order', '>=', date_start), ('date_order', '<=', date_end)])

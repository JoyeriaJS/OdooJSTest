...
class PosConfig(models.Model):
    ...
    def _get_group_pos_manager(self):
        return self.env.ref('punto_de_venta.group_pos_manager')

    def _get_group_pos_user(self):
        return self.env.ref('punto_de_venta.group_pos_user')

    def _get_default_tip_product(self):
        tip_product_id = self.env.ref("punto_de_venta.product_product_tip", raise_if_not_found=False)
    ...
    def _reset_default_on_vals(self, vals):
        if 'tip_product_id' in vals and not vals['tip_product_id'] and 'iface_tipproduct' in vals and vals['iface_tipproduct']:
            default_product = self.env.ref('punto_de_venta.product_product_tip', False)
            if default_product:
    ...
    def _update_preparation_printers_menuitem_visibility(self):
        prepa_printers_menuitem = self.sudo().env.ref('punto_de_venta.menu_pos_preparation_printer', raise_if_not_found=False)
    ...
    def _force_http(self):
        enforce_https = self.env['ir.config_parameter'].sudo().get_param('punto_de_venta.enforce_https')
    ...
    def get_limited_product_count(self):
        default_limit = 20000
        config_param = self.env['ir.config_parameter'].sudo().get_param('punto_de_venta.limited_product_count', default_limit)
        try:
    ...
    def toggle_images(self, for_products, for_categories):
        self.env['ir.config_parameter'].sudo().set_param('punto_de_venta.show_product_images', for_products)
        self.env['ir.config_parameter'].sudo().set_param('punto_de_venta.show_category_images', for_categories)
    ...
    def _get_special_products(self):
        default_discount_product = self.env.ref('punto_de_venta.product_product_consumable', raise_if_not_found=False) or self.env['product.product']
        default_tip_product = self.env.ref('punto_de_venta.product_product_tip', raise_if_not_found=False) or self.env['product.product']

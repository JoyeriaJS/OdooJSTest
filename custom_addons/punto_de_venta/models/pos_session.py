...
class PosSession(models.Model):
    ...
    def _compute_cash_control(self):
        # Only one cash register is supported by punto_de_venta.
        for session in self:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            update_stock_at_closing = pos_config.company_id.punto_de_venta_update_stock_quantities == "closing"
        ...
        if self.user_has_groups('punto_de_venta.group_pos_user'):
    ...
    def _validate_session(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        ...
        self.ensure_one()
        data = {}
        sudo = self.user_has_groups('punto_de_venta.group_pos_user')
    ...
    def get_closing_control_data(self):
        if not self.env.user.has_group('punto_de_venta.group_pos_user'):
        ...
        return {
            ...
            'is_manager': self.user_has_groups("punto_de_venta.group_pos_manager"),
    ...
    def action_view_order(self):
        return {
            ...
            'view_mode': 'tree,form',
            'views': [
                (self.env.ref('punto_de_venta.view_pos_order_tree_no_session_id').id, 'tree'),
                (self.env.ref('punto_de_venta.view_pos_pos_form').id, 'form'),
                ],
            'type': 'ir.actions.act_window',
        ...
        for session in sessions:
            if self.env['mail.activity'].search_count([('res_id', '=', session.id), ('res_model', '=', 'pos.session')]) == 0:
                session.activity_schedule(
                    'punto_de_venta.mail_activity_old_session',
    ...
    def _pos_data_process(self, loaded_data):
        ...
        loaded_data['show_product_images'] = self.env['ir.config_parameter'].sudo().get_param('punto_de_venta.show_product_images', 'yes')
        loaded_data['show_category_images'] = self.env['ir.config_parameter'].sudo().get_param('punto_de_venta.show_category_images', 'yes')
        ...
        return {
            'search_params': {
                ...
                'fields': [
                    ...
                    'country_id', 'state_id', 'tax_calculation_rounding_method', 'nomenclature_id', 'punto_de_venta_use_ticket_qr_code',
                    'punto_de_venta_ticket_unique_code', 'account_fiscal_country_id',
                ],
            }
        }
    ...
    def _get_pos_fallback_nomenclature_id(self):
        ...
        fallback_nomenclature_id = self.env['ir.config_parameter'].sudo().get_param('punto_de_venta.fallback_nomenclature_id')
    ...
    def _load_onboarding_data(self):
        if not self.env.user.has_group("punto_de_venta.group_pos_user"):
            raise AccessDenied()
        convert.convert_file(self.env, 'punto_de_venta', 'data/punto_de_venta_onboarding.xml', None, mode='init', noupdate=True, kind='data')
        shop_config = self.env.ref('punto_de_venta.pos_config_main', raise_if_not_found=False)
    ...
    def _load_onboarding_main_config_data(self, shop_config):
        convert.convert_file(self.env, 'punto_de_venta', 'data/punto_de_venta_onboarding_main_config.xml', None, mode='init', noupdate=True, kind='data')
    ...
    def _after_load_onboarding_data(self):
        config = self.env.ref('punto_de_venta.pos_config_main', raise_if_not_found=False)
        if config:
            config.with_context(bypass_categories_forbidden_change=True).write({
                'limit_categories': True,
                'iface_available_categ_ids': [Command.link(self.env.ref('punto_de_venta.pos_category_miscellaneous').id), Command.link(self.env.ref('punto_de_venta.pos_category_desks').id), Command.link(self.env.ref('punto_de_venta.pos_category_chairs').id)]
            })
    ...
    def _is_capture_system_activated(self):
        ...
        return str2bool(self.env['ir.config_parameter'].sudo().get_param('punto_de_venta.capture_unprocessed_order', True))
